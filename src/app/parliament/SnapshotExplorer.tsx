...
const SnapshotInner = ({ index, initialDate }: { index: ParliamentIndexEntry[]; initialDate?: string }) => {
  const { reset } = useParliamentFilters();
  const [selectedDate, setSelectedDate] = useState<string | undefined>(() => {
    if (initialDate) return initialDate;
    try { return localStorage.getItem(LOCAL_KEY) || index[index.length - 1]?.date; } catch { return index[index.length - 1]?.date; }
  });
  const [snapshot, setSnapshot] = useState<ParliamentSnapshot | null>(null);
  const [loadingSnapshot, setLoadingSnapshot] = useState(false);
  const [partyMetaMap, setPartyMetaMap] = useState<Record<string, { leaning: 'left' | 'center' | 'right' }>>({});
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="text-sm text-muted max-w-2xl">Historical snapshots of the chamber. Select a term start date to view its composition; filters apply per snapshot.</p>
        </div>
        <div className="flex gap-2 items-end">
          <label className="form-control w-56">
            <div className="label py-1"><span className="label-text text-xs uppercase tracking-wide">Snapshot Date</span></div>
            <select className="select select-sm select-bordered" value={selectedDate} onChange={e => setSelectedDate(e.target.value)}>
              {index.slice().sort((a,b) => b.date.localeCompare(a.date)).map(entry => (
                <option key={entry.date} value={entry.date}>{entry.date.slice(0,10)}</option>
              ))}
            </select>
          </label>
        </div>
      ...
      {snapshot && !loadingSnapshot && (
        <div className="grid gap-8 lg:grid-cols-4 items-start">
          <div className="lg:col-span-3 order-2 lg:order-1 card-surface">
            <p className="text-xs text-muted mb-3">Snapshot date: {snapshot.meta.date} · Total members: {snapshot.meta.total}{loadingMeta && ' · Loading party meta…'}</p>
            <HemicycleReact members={snapshot.members} partyMetaOverride={partyMetaMap} />
          </div>
...
const SnapshotExplorer = ({ initialDate }: SnapshotExplorerProps) => {
  ...
  if (!index || index.length === 0) return <div className="p-4 text-sm">No snapshots indexed. Run <code>npm run snapshot:range:terms</code>.</div>;
