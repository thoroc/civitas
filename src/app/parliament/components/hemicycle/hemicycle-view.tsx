...
import HemicycleExportBar from './hemicycle-export-bar';
import HemicycleSeats from './hemicycle-seats';
import HemicycleTooltip from './hemicycle-tooltip';
...
interface HemicycleDiagramProps {
  svgRef: React.RefObject<SVGSVGElement>;
  pad: number;
  vbWidth: number;
  vbHeight: number;
...
const HemicycleInstructions = () => (
  <div id='hemicycle-instructions' className='sr-only'>
    Interactive hemicycle: Use Arrow keys to move between seats. Home and End
    jump to first or last seat. Page Up and Page Down move by ten seats. Press
    Enter to repeat the current seat announcement.
  </div>
);
...
const HemicycleDiagram = ({
  ...
  >
    ...
    {filteredMembers.length === 0 && (
      <EmptyStateOverlay pad={pad} vbWidth={vbWidth} vbHeight={vbHeight} />
    )}
  </svg>
);
