...
interface PartyInfo { id: string; label: string; color: string; count: number; total: number; }
...
const PartyLegend = ({ members }: PartyLegendProps) => {
  ...
  for (const m of members) {
    ...
    if (existing) existing.total += 1; else totalMap.set(id, { id, label, color, count: 0, total: 1 });
  }
  ...
  const parties = Array.from(totalMap.values()).sort((a, b) => b.total - a.total || a.label.localeCompare(b.label));
        ...
        return (
          <li key={p.id} className="flex items-center justify-between gap-3 group rounded-md px-2 py-1 hover:bg-base-200/80 transition-colors">
            ...
            <span className="text-xs tabular-nums flex items-center gap-1">
              <span className={reduced ? 'text-primary font-medium' : 'text-muted'}>{p.count}</span>
