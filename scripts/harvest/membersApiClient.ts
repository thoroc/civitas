...
interface RawPartySpell {
  PartyId: string;
  Party: string;
  Start: string;
  End?: string;
}
interface RawSeatSpell {
...
export async function harvestMembers(
  ...
  while (true) {
    ...
    const batch: RawSearchMember[] = Array.isArray(rawItems)
      ? rawItems.map((it: any) => ({
          MemberId: it?.value?.id || it?.MemberId || it?.Id || it?.id,
          Name:
            it?.value?.nameDisplayAs ||
            it?.Name ||
            it?.DisplayAs ||
            it?.name ||
            '',
        ...
        try {
          ...
          if (enableHistory) {
            try {
              ...
              console.debug(
                `[harvest][history][dbg] history fetch failed member=${m.memberId} msg=${e.message || e}`
              );
            }
          }
          ...
          try {
            ...
            const partiesRaw: RawPartySpell[] = await cachedGet(
              partyUrl,
              cacheCfg
            );
            ...
            for (const container of partyContainers) {
              if (Array.isArray(container)) {
                for (const raw of container as any[]) {
                  const pid =
                    raw?.value?.id || raw?.PartyId || raw?.Id || raw?.id;
                  const name =
                    raw?.value?.name ||
                    raw?.value?.nameDisplayAs ||
                    raw?.Party ||
                    raw?.Name ||
                    raw?.nameDisplayAs;
                  const start =
                    ...
                    raw?.value?.start;
                  const end =
                    raw?.End || raw?.end || raw?.value?.End || raw?.value?.end;
                  if (pid && name && start) {
                    partySpells.push({
                      memberId: m.memberId,
          ...
          if (partySpells.filter(p => p.memberId === m.memberId).length === 0) {
            ...
            if (lp && lhm?.membershipStartDate) {
              ...
              console.debug(
                `[harvest][dbg] derived party spell member=${m.memberId} party=${lp.name}`
              );
            } else {
              console.debug(
                `[harvest][dbg] no party spells parsed member=${m.memberId} keys=${Object.keys(detail || {})} valueKeys=${detail?.value ? Object.keys(detail.value) : []}`
              );
            }
          }

          const incumbencies: RawSeatSpell[] =
            detail?.value?.Incumbencies || detail?.Incumbencies || [];
          if (incumbencies.length === 0) {
            ...
            if (lhm?.membershipStartDate && lhm?.membershipFromId) {
              seatSpells.push({
                ...
                constituencyName:
                  lhm.membershipFrom || String(lhm.membershipFromId),
                start: lhm.membershipStartDate,
                end: lhm.membershipEndDate || undefined,
                provisional: true,
              });
              console.debug(
                `[harvest][dbg] derived seat spell member=${m.memberId} constituency=${lhm.membershipFrom}`
              );
            } else {
              console.debug(
                `[harvest][dbg] no seat incumbencies member=${m.memberId}`
              );
            }
          }
          for (const inc of incumbencies) {
            if (!inc.ConstituencyId && typeof inc.Constituency === 'object') {
              ...
              const cname =
                ...
                cObj?.nameDisplayAs ||
                cObj?.value?.name ||
                cObj?.value?.nameDisplayAs;
              if (cid) inc.ConstituencyId = cid;
              if (cname) (inc as any).Constituency = cname;
            }
            if (!inc.Start && (inc as any).start)
              inc.Start = (inc as any).start;
          ...
          console.warn(
            `[harvest] failed member ${m.memberId}: ${e.message || e}`
          );
        }
      })
    )
