...
async function main() {
    ...
    for (const [mid, arrRaw] of Array.from(byMember.entries())) {
      ...
      for (const s of arr) {
        if (s.end && s.end < s.start) {
          negatives.push(
            `${label} negative member=${mid} (${s.start}-${s.end})`
          );
        }
      }
      for (let i = 1; i < arr.length; i++) {
        ...
        if (!prev.end || prev.end > curr.start) {
          overlaps.push(
            `${label} overlap member=${mid} prev(${prev.start}-${prev.end || ''}) curr(${curr.start}-${curr.end || ''})`
          );
          ...
          gaps.push(
            `${label} gap member=${mid} prevEnd=${prev.end} nextStart=${curr.start} days=${gapDays}`
          );
        }
      }
    }
  ...
  const anyIssues = ['overlaps', 'negatives', 'gaps'].some(
    k => (partyValidation as any)[k].length || (seatValidation as any)[k].length
  );
  if (anyIssues) {
    console.warn(
    ...
    const logSample = (title: string, arr: string[]) => {
      if (arr.length) {
        console.warn(`  ${title}:`);
